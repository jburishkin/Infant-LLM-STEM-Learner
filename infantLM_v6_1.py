#!/usr/bin/env python3
"""
Infant LLM v6.1 — STEM Learner (local mode, no SSO)
- Math reasoning: detects & evaluates expressions in natural queries (e.g., "what is 2+3=")
- Memory: "memory <type>" lists all items in that category (math/science/coding/engineering/technology/general)
- Clear memory endpoint + button
- Same UI controls (personality, temperature, type-preference), persistent memory, FAISS search
"""
...
# ---------------- Memory store ----------------
class MemoryStore:
    def __init__(self, dim: int, path_dir="infant_data_v6_1"):
        self.dim = dim
...
class Infant:
    ...
    def classify(self, text: str) -> str:
        ...
        if self.MATH_RE.match(t) or t.endswith("=") or (any(ch in t for ch in "+-*/^=") and any(c.isdigit() for c in t)):
            return "math"
        code_hints = ("def ", "class ", "print(", "function ", "{", "};", "console.log", "for(", "while(")
        sci_hints = ("atom","cell","molecule","h2o","gravity","boil","°c","celsius","kelvin","force","mass","energy","dna","photosynthesis")
        tech_hints= ("server","http","api","database","linux","kernel","python","java","c++","network","router","switch","gpu","cpu","pci","usb")
        eng_hints = ("torque","beam","load","stress","strain","voltage","current","ohm","watt","newton","pascal","bridge","circuit","gear")
        if any(h in t for h in code_hints): return "coding"
        if any(h in t for h in eng_hints):  return "engineering"
        if any(h in t for h in tech_hints): return "technology"
        if any(h in t for h in sci_hints):  return "science"
        return "general"
    ...
    def try_eval_math(self, text: str):
        """
        Extract & evaluate a math expression anywhere in the text.
        Supports forms like 'what is 2+3=' or '5*(4+1)=' or '2+2'.
        """
        # find the longest math-like token sequence
        candidates = re.findall(r"[0-9\.\s+\-*/()^%=]+", text)
        if not candidates:
            return None
        expr = max((c.strip() for c in candidates), key=len).rstrip("=")
        if not expr:
            return None
        try:
    ...
    def recall(self, text: str, k=6, typ: Optional[str] = None):
    ...
    def generate(self, prompt: str, temperature: float = 0.9, max_len: int = 40):
        ...
        with torch.no_grad():
            for _ in range(max_len):
                ...
                if n == 2:  # <eos>
                    break
                out_words.append(self.idx.get(n, "<unk>"))
...
# load external HTML file if present; else fallback to inline minimal banner
HTML_PATH = "infant_ui_v6_1.html"
if os.path.exists(HTML_PATH):
    with open(HTML_PATH, "r", encoding="utf-8") as f:
        HTML = f.read()
else:
    HTML = "<h2>Infant LLM v6.1 — UI file missing</h2>"
...
def chat():
    ...
    # Memory commands — category or semantic
    ...
    # Math reasoning first (extract expression from natural text)
    m_eval = infant.try_eval_math(msg)
    if m_eval is not None:
        expr, val = m_eval
        return jsonify({"reply": f"{expr} = {val}", "memory": []})

    # Otherwise: retrieval-augmented generation
