...
def memlist():
    items = []
    # Pull the last 30 memory entries
    if infant.mem.texts:
        for t, ty in list(zip(infant.mem.texts, infant.mem.types))[-30:]:
            items.append({"text": t, "type": ty})

    # Load session for maturity and stage
    ses = load_session()
...
def chat():
    ...
    if msg.lower().startswith("learn "):
        ...
        # --- Ultra-slow stage-scaled growth ---
        stage_rate = {
            "infant": 0.05,
            "child": 0.08,
            "teenager": 0.10,
        ...
        rate = stage_rate.get(stage, 0.05)
        div_bonus = min(diversity, 5) * 0.5

        # Slow down by roughly 10× overall
        maturity = int(min(100, round((((learned * rate) + div_bonus) / target * 100) / 10)))
        # --------------------------------------
        ...
        save_session(ses)
        return jsonify({
            "reply": f"Learned ({ty}) loss≈{loss:.3f}",
            "updated_memory": True,
            "stage": stage,
            "suggest_growth": suggest
        })
